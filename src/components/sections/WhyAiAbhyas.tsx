...
export function WhyAiAbhyas() {
  const Card = ({ card, heightClass, delay }: { card: typeof cards[0], heightClass: string, delay: number }) => (
    ...
    >
       ...
       <div className="absolute inset-0 p-6 flex flex-col justify-end text-white transition-opacity duration-500 group-hover:opacity-0 z-10">
          <div className="flex items-center gap-3">
            <card.icon className="w-6 h-6 text-primary shrink-0" />
            <h3 className="text-[14px] font-heading font-bold leading-tight drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">{card.title}</h3>
          </div>
       </div>

       {/* Hover State (Heading, Description, Button) sliding from right */}
       <div className="absolute inset-0 bg-primary/95 p-6 flex flex-col text-primary-foreground transform translate-x-full opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] z-10">
          <h3 className="text-[18px] font-heading font-bold mb-4 leading-tight">{card.title}</h3>
          <p className="text-sm opacity-90 mb-auto leading-relaxed">{card.description}</p>
          <Link href="/courses" className="mt-auto">
            <button className="bg-background text-foreground px-5 py-3 rounded-xl text-sm font-semibold hover:bg-muted transition-colors shadow-sm w-max">
              Explore Courses
            </button>
          </Link>
       </div>
    ...
    <section className="py-16 md:py-24 relative overflow-hidden bg-background">
      <div className="container mx-auto px-4 md:px-6 relative z-10 max-w-7xl">
        <div className="text-center max-w-4xl mx-auto mb-16 px-4 flex flex-col items-center">
          ...
          >
            We go beyond generic tutorials to deliver an outcome-based experience.
          </motion.p>
        </div>
