...
export function CourseCard({ course }: { course: CourseData }) {
  ...
  return (
    <div className="group relative flex flex-col bg-card rounded-[24px] border border-border overflow-hidden transition-all duration-300 hover:-translate-y-[6px] hover:border-primary/50 hover:shadow-[0_20px_40px_-15px_rgba(139,92,246,0.12)] dark:hover:shadow-[0_20px_40px_-15px_rgba(139,92,246,0.25)] h-full">
      
      {/* 1. Cover Image Block */}
      <div className="relative w-full h-[170px] md:h-[190px] shrink-0 overflow-hidden rounded-t-[24px] bg-muted/30">
        <Image 
          src={course.image}
          alt={course.title}
          fill
        ...
        <div className="absolute top-3 left-3 z-10">
          <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full whitespace-nowrap ${levelColor}`}>
            {course.level}
          </span>
        </div>
        <div className="absolute top-3 right-3 z-10">
          <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full border border-[#14B8A6]/20 whitespace-nowrap ${modeColor}`}>
            {course.mode}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        ...
        <div className="flex flex-col gap-1.5">
          <h3 className="text-lg font-heading font-bold text-foreground line-clamp-2 leading-tight group-hover:text-primary transition-colors h-[44px]">
            {course.title}
          </h3>
        ...
        <div className="flex flex-wrap gap-1.5 h-[24px] overflow-hidden">
          {course.skills.slice(0, 3).map((skill, idx) => (
            <span key={idx} className="px-2 py-0.5 text-[10px] font-medium bg-muted text-foreground rounded-full whitespace-nowrap">
              {skill}
            </span>
          ))}
        </div>
        ...
        <Link href={`/courses/${course.id}`} className="w-full mt-1">
          <Button className="w-full h-10 text-[13px] font-semibold group/btn">
            Explore Course <ArrowRight className="ml-1.5 w-3.5 h-3.5 transition-transform duration-300 group-hover/btn:translate-x-1" />
          </Button>
        </Link>
